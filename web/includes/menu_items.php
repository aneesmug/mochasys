...
<?php
    $query_cat = mysqli_query($conDB, "SELECT * FROM `menu_category` WHERE `id`='".$_GET['cat']."' ");
    	while ($row = mysqli_fetch_assoc($query_cat)) {
    		$menu_category = $row['name_eng'];
    	}
                        ...
                        >
                            <div class="elementor-column-wrap elementor-element-populated">
                                <div class="elementor-widget-wrap">
                                    ...
                                    >
                                        <div class="elementor-widget-container">
                                        <h1 class="elementor-heading-title elementor-size-default"><?=$menu_category?></h1>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            >
                <div class="elementor-container elementor-column-gap-default">
                    <div class="elementor-row row">
                    	<?php                            
                            $query_items = mysqli_query($conDB, "SELECT * FROM `menu_item` WHERE `category_id`='".$_GET['cat']."' AND `price_level`='1' ");
                            while ($row = mysqli_fetch_array($query_items)) {
                        ?>
                        <!-- start item -->
                        <div
                            class="elementor-column elementor-col-50 elementor-top-column elementor-element elementor-element-fa86279 row-cols-6"
                            data-id="fa86279"
                            data-element_type="column"
                            data-settings='{"craftcoffee_ext_is_scrollme":"false","craftcoffee_ext_is_smoove":"false","craftcoffee_ext_is_parallax_mouse":"false","craftcoffee_ext_is_infinite":"false","craftcoffee_ext_is_fadeout_animation":"false"}'
                        >
                            <div class="elementor-column-wrap elementor-element-populated">
                           	...
                           	<table class="table" style="border: 1px solid #ccc !important; ">
								<tr>
									<th colspan="4">
										<h3 class="food-menu-title"><?=$row['name_eng']?></h3>
									</th>
								</tr>
								<tr>
									<th rowspan="3" style="width: 0px !important;" class="imgz">
										<div class="food-menu-img" style="margin-left: 0 !important;">
											<img src="./../public/app-assets/assets/item_img/<?=$row['image'];?>" style="width: 80px !important; height: 80px !important;" />
										</div>
									</th>
									<th>Size</th>
									<td>
								...
								<tr>
									<th>Calories</th>
									<td><?=($row['big_cal']) ? "".$row['big_cal']."<small>-cal</small>":"";?></td>
									<td><?=($row['small_cal']) ? "".$row['small_cal']."<small>-cal</small>":"";?></td>
								</tr>
								<tr>
	                    			<th>Price</th>
	                    			<td>
	                    				<?=($row['big_price']) ? "".$row['big_price']."<small>-SR</small>":"";?>
	                    			</td>
	                    			<td>
	                    				<?=($row['small_price']) ? "".$row['small_price']."<small>-SR</small>":"";?>
	                    			</td>
	                    		</tr>
							</table>                                        
                                </div>
